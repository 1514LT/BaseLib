...
{
  std::string path = "/home/" + std::string(std::getenv("USER")) + "/dataacquisition";
...
{
  std::string result = read_file_contents(getHomePath() + "/defaults.json");
  Json::Value root = StringToJson(result);
  std::string path = root["defaultPath"].asString();
  check_and_create_directory(path);
  return path;
}
...
std::string JRLC::getAutoRunPath()
{
  std::string file_path = JRLC::getUserPath() + "/dataacquisition/";
  JRLC::check_and_create_directory(file_path);
  file_path += "autoRun/";
...
void JRLC::showJson(Json::Value root)
{
  std::string result = JsonToString(root);
  // printf("MSG_Type:%s\njsonValue:%s\n",root["MSG_Type"].asString().c_str(),result.c_str());
}